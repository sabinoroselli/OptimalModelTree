...
def optimal_OMT(df, features, labels, Splits, C, config, solver):
    ...
    d = {
        t: m.IntVar(0, 1, f'd[{t}]')
        for t in T_B
    }
    # oblique split coefficients
    a = {
        (j, t): m.NumVar(-m.infinity(), m.infinity(), f'a[{j},{t}]')
        for j in features
        for t in T_B
    }
    # |a|
    a_abs = {
        (j, t): m.NumVar(0, m.infinity(), f'a_abs[{j},{t}]')
        for j in features
        for t in T_B
    }
    # sparsity indicators
    s = {
        (j, t): m.IntVar(0, 1, f's[{j},{t}]')
        for j in features
        for t in T_B
    }
    ...
    # Const_01 : s >= |a|
    # NOTE:
    # Since s is binary, this only works correctly if |a| <= 1.
    # If a can exceed 1 in magnitude, replace with:
    #
    #     a_abs[j,t] <= M_coeff * s[j,t]
    #
    # -------------------------------------------------------------------------
    for j in features:
        for t in T_B:
            m.Add(a_abs[j, t] <= s[j, t])
    ...
    for t in T_B:
        m.Add(
            m.Sum(a_abs[j, t] for j in features) <= d[t]
        )
    ...
    for t in T_B:
        m.Add(
            m.Sum(s[j, t] for j in features) >= d[t]
        )

...
def optimal_OMT(df, features, labels, Splits, C, config):
    ...
    Const_16 = m.addConstrs(
        (z[i, t] == 1)
        >>
        (quicksum([Beta[j, t] * df.loc[i, j] for j in features]) + Delta[t] - df.loc[i, labels[0]] == e[i, t])
        for i in I
        for t in T_L
    )
    ...
    Const_19 = m.addConstr( quicksum([d[t] for t in T_B]) <= Splits )


    m.setObjective(
        quicksum([Bet_abs[f, t] for f in features for t in T_L]) + C * quicksum([e_abs[i, t] for i in I for t in T_L])
    )
